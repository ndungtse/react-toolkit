 ...
 */
const VariableInputs = (props: Props) => {
   const { children, className, rightSection, leftSection, addSection, data } = props;
         ...
         {inputRows.map((r, i) => (
            <div key={i} className={cn('flex w-full gap-2')}>
               ...
               <button onClick={() => removeInput(i)} className="p-0 flex items-end">
                  {rightSection ?? (
                     <span className="flex flex-col mt-auto items-center justify-center w-11 border text-2xl p-2">-</span>
                  )}
               </button>
            </div>
         ))}
         {addSection ?? (
            <button onClick={addInput} className="flex border-2 px-5 w-fit mx-auto flex-col text-2xl p-1.5">
               +
            </button>
         )}
      </div>
